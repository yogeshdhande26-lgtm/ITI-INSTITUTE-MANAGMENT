...
export const LessonPlanList: React.FC<LessonPlanListProps> = ({ onNewEntry, onEditEntry }) => {
  ...
  const exportConsolidatedExcel = () => {
    const data = [
      ["Lesson Plan Register (Consolidated)"],
      [],
      ["Date", "Month", "Year", "Trade", "Batch", "Instructor", "Lesson Name", "Lesson No", "Duration"],
      ...plans.map(p => [
        p.plannedDate || '-',
        safeFormatDate(p.plannedDate, 'MMMM'),
        safeFormatDate(p.plannedDate, 'yyyy'),
        p.trade,
        p.batch,
        p.instructorName,
        p.lessonName,
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Lesson Register");
    XLSX.writeFile(wb, "Full_Lesson_Plan_Register.xlsx");
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'lesson_plans', deleteConfirmId));
      toast.success('Lesson plan deleted successfully');
      setDeleteConfirmId(null);
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, `lesson_plans/${deleteConfirmId}`);
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-4xl font-extrabold tracking-tight text-slate-900">Lesson Plans</h2>
          <p className="text-slate-500 mt-2 font-medium">Technical Lesson Plan Register for ITI Instructors.</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button onClick={onNewEntry} className="gap-2 bg-slate-900 hover:bg-slate-800 shadow-lg h-12 px-6 rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98]">
            <Plus className="h-5 w-5" /> New Lesson Plan
          </Button>
        </div>
      </div>
      ...
      {loading ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-slate-900 font-semibold">
                        ...
                        <span className="capitalize">{plan.duration} {plan.durationUnit}</span>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-1">
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-white rounded-3xl border-2 border-dashed border-slate-200 px-6 text-center">
          ...
          <h3 className="text-2xl font-bold text-slate-900">No lesson plans found</h3>
          <p className="text-slate-500 mb-8 max-w-sm">Your technical lesson plan register is empty. Start by creating your first plan using our AI-powered tool.</p>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={(open) => !open && setDeleteConfirmId(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This action cannot be undone. This will permanently delete this lesson plan record.
            </DialogDescription>
          </DialogHeader>
