...
export const MonthlyPlanList: React.FC<MonthlyPlanListProps> = ({ onNewEntry, onEditEntry }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      handleFirestoreError(error, OperationType.LIST, 'monthly_plans');
      setLoading(false);
    });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'monthly_plans', deleteConfirmId));
      toast.success('Monthly plan deleted successfully');
      setDeleteConfirmId(null);
    } catch (error) {
      handleFirestoreError(error, OperationType.DELETE, `monthly_plans/${deleteConfirmId}`);
    } finally {
      setIsDeleting(false);
    }
  };

  const exportYearRegisterExcel = () => {
    const data = [
      ["Year-wise Monthly Plan Register"],
      [],
      ["Year", "Month", "Trade", "Total Lessons", "Institute"],
      ...plans.map(p => [
        p.year,
        p.month,
        p.trade,
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Year Register");
    XLSX.writeFile(wb, "Year_Wise_Monthly_Plan_Register.xlsx");
    toast.success('Yearly register exported to Excel');
  };
  ...
  const totalYearLessons = plans.reduce((acc, p) => acc + (Number(p.lessonTotal) || 0), 0);

  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-4xl font-extrabold tracking-tight text-slate-900 text-marathi">मासिक नियोजन रजिस्टर</h2>
          <p className="text-slate-500 mt-2 font-medium">Monthly Plan Register for Academic Tracking.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" onClick={exportYearRegisterExcel} className="gap-2 border-orange-200 text-orange-700 hover:bg-orange-50 h-12 px-6 rounded-xl">
            <FileSpreadsheet className="h-5 w-5" /> Export Year Register
          </Button>
          <Button onClick={onNewEntry} className="gap-2 bg-slate-900 hover:bg-slate-800 shadow-lg h-12 px-6 rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98]">
            <Plus className="h-5 w-5" /> New Monthly Plan
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-slate-900 text-white border-none shadow-xl rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Total Plans</p>
        ...
        <Card className="bg-emerald-600 text-white border-none shadow-xl rounded-2xl">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-emerald-200 uppercase tracking-widest mb-1">Total Lessons</p>
                <p className="text-3xl font-black">{totalYearLessons}</p>
              </div>
              <div className="h-12 w-12 bg-white/10 rounded-xl flex items-center justify-center">
      ...
      <div className="relative group">
        ...
        <input 
          type="text"
          placeholder="Search by trade, month, year or institute..."
      ...
      {loading ? (
            ...
            <Card key={plan.id} className="overflow-hidden group hover:shadow-xl transition-all border-slate-200 rounded-2xl flex flex-col">
              <CardHeader className="pb-4 flex flex-row items-start justify-between space-y-0 bg-slate-50/50">
                <div className="space-y-2">
                  ...
                  <CardTitle className="text-xl font-bold line-clamp-1 text-slate-900">{plan.month} {plan.year}</CardTitle>
                </div>
                <DropdownMenu>
                  <DropdownMenuTrigger
                    render={
                ...
                <div className="space-y-4 text-sm text-slate-600 flex-1">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Planned Lessons</span>
                      <span className="text-2xl font-black text-slate-900">{plan.lessonTotal || 0}</span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Planned Weeks</span>
                      <span className="text-2xl font-black text-slate-900">{plan.weeks?.length || 0}</span>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex flex-col items-center justify-center py-32 bg-white rounded-3xl border-2 border-dashed border-slate-200 px-6 text-center">
          ...
          <h3 className="text-2xl font-bold text-slate-900">No monthly plans found</h3>
          <p className="text-slate-500 mb-8 max-w-sm">Start organizing your academic year by creating your first monthly plan record.</p>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={(open) => !open && setDeleteConfirmId(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Delete Monthly Plan?</DialogTitle>
            <DialogDescription>
              This action cannot be undone. This will permanently delete this monthly plan record.
            </DialogDescription>
          </DialogHeader>
